...
#. Clone the GitHub repository to a mirror repository at the `NERSC GitLab
   instance <https://software.nersc.gov/>`__ (where we have direct access to the
   *Cori* and *Perlmutter* machines).

#. Implement a CI workflow for our repository using GitLab's builtin CI tools
...
from a GitHub repository. For our repositories' example workflow, the goal is
...
securely through our CI pipeline.
